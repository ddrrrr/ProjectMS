...
{
    static class fftbase
    {
        public static void fftProcess(double[] data, out double[] result)
        {
            int len = data.Length;
            double[] data_r = data;
            ...
            for (int i = 0; i < len; i++)
                data_i[i] = result_r[i] = result_i[i] = result[i] = 0;
            Dit2_FFT(ref data_r, ref data_i, ref result_r, ref result_i);
            for (int i = 0; i < len / 2; i++)
                result[i] = (Math.Pow(result_r[i], 2) + Math.Pow(result_i[i], 2)) / len;
        }
